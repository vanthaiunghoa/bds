...
class TableCompute(object):
    ...
    def process(self, products, ppg=PPG):
        ...
        x = 0
        for p in products:
#             x = min(max(p.website_size_x, 1), PPR)  # tu sua
#             y = min(max(p.website_size_y, 1), PPR)
            ...
            self.table[pos // PPR][pos % PPR] = {
                'product': p, 'x': x, 'y': y,
#                 'class': " ".join(x.html_class for x in p.website_style_ids if x.html_class) # tu sua
                'class': ""
...
#         return 'website_published desc,%s , id desc' % post.get('order', 'website_sequence desc')
    ...
    @http.route([
        '/bds',
        '/bds/page/<int:page>',
        '/bds/category/<model("product.public.category"):category>',
        '/bds/category/<model("product.public.category"):category>/page/<int:page>'
...
#         Product = request.env['product.template']
        ...
        return request.render("bds.products", values)

    @http.route(['/bds/product/<model("bds.bds"):product>'], type='http', auth="public", website=True)
    def product(self, product, category='', search='', **kwargs):
        ...
        return request.render("bds.product", values)
